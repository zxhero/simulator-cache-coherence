...
void handle_msg_fromBUS_MESI(struct cache_block* block, struct msg* msg,struct L1_cache *cache,long int cycle){
	if(block == NULL){
		...
		if(msg -> operation == BUSRDX){
			...
			send_message(msg,msg->cycle+1,SUCCESS,0,msg->addr,BROADCAST,cache->id,cache->pipe_to_bus); 
		}
		...
		if(msg -> operation == BUSRD){ //Busread, Exclusive 
			...
			send_message(msg,msg->cycle+1,SUCCESS,0,msg->addr,BROADCAST,cache->id,cache->pipe_to_bus); //Send bus read
		}
			...
			send_message(msg,msg->cycle+1,FLUSH,block,msg->addr,BROADCAST,cache->id,cache->pipe_to_bus); //Sends back success to the bus
		}
	}
	else if(block -> status == SHARED){
		if(msg -> operation == BUSRD){
			...
			send_message(msg,msg->cycle+1,SUCCESS,0,msg->addr,BROADCAST,cache->id,cache->pipe_to_bus); //Send bus read X
		}
	}
	else if(block -> status == MODIFY){
		if(msg -> operation == BUSRD){
